...
exports.middleware = store => next => action => {
	...
	// Quick hack for auto-layout on startup.
	if (autoLayout && uid) {
		autoLayout = false
		setTimeout(() => {
			runCommand(uid, 'hyperlayout')
		})
	}
	...
	if (type === 'SESSION_ADD_DATA') {
		...
		if (testedData && testedData[1]) {
			// Quick fix to clean up data.
			const d = testedData[1]
			const config = JSON.parse(d.substring(0, d.lastIndexOf('}') + 1))
